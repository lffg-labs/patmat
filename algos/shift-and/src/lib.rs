...
mod tests {
    ...
    #[test]
    fn basic_test() {
        let mut s = ShiftAnd::new(b"abcABCabc", b"A");
        assert_eq!(s.search().unwrap(), 3);

        let mut s = ShiftAnd::new(b"abcABCabc", b"Z");
    ...
    #[test]
    fn multiple_occurrences_tests() {
        let cases = &[
            ("BABAB", "A", [1, 3].as_slice()),
            ("XXabcYYabcZZabc", "abc", &[2, 7, 12]),
        ];
        for (i, (text, pat, expected_list)) in cases.iter().enumerate() {
            println!("running test [{i}] for text [{text}] and pattern [{pat}]");
            let mut s = ShiftAnd::new(text.as_bytes(), pat.as_bytes());
            for expected in *expected_list {
                assert_eq!(s.search().unwrap(), *expected);
            }
            assert_eq!(s.search(), None);
        }
    }
    ...
    fn go_lang_tests() {
        ...
        for (i, (text, pat, expected)) in cases.iter().enumerate() {
            println!("running test [{i}] for text [{text}] and pattern [{pat}]");
            let mut s = ShiftAnd::new(text.as_bytes(), pat.as_bytes());
